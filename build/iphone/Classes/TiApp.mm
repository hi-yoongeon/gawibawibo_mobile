...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kgawibawibo_mobileUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kgawibawibo_mobileUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}

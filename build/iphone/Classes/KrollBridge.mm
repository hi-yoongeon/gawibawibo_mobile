...
@implementation gawibawibo_mobileObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_gawibawibo_mobile);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any gawibawibo_mobile patches in JS before a context is loaded... nice for 
...
{
	// create gawibawibo_mobile global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_gawibawibo_mobile = [[gawibawibo_mobileObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_gawibawibo_mobile];
	
	NSString *_gawibawibo_mobileNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _gawibawibo_mobileNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_gawibawibo_mobile valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.gawibawibo_mobile.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
